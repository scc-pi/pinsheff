...
To use GitHub with RStudio there's a [cheatsheet](https://github.com/rstudio/cheatsheets/blob/main/git-github.pdf) and a thorough step-by-step guide, [Happy Git with R](https://happygitwithr.com/) by Jenny Bryan. The main recommendation is to setup the connection using keys for [SSH](https://happygitwithr.com/ssh-keys.html). This connection method by-passes issues with the Council's proxy server and using the HTTPS connection method. Since August 2021 [GitHub token authentication is required](https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/) (e.g. SSH) and some of the steps in the Jenny Bryan's book (e.g. chapters 9 and 10 on connecting and HTTPS) are out of date. As of October 2021 this is the case, but if [issue \#181](https://github.com/jennybc/happy-git-with-r/issues/181) gets closed in the [Happy Git with R GitHub repo](https://github.com/jennybc/happy-git-with-r), then it's been fixed. Until then the recommendation is to skip chapters 9 and 10 and focus on 11 and 15 (SSH and New project, GitHub first).
