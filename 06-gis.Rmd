...
This chapter is mainly aimed at our data analysts with little or no experience of spatial data and GIS. GIS has traditionally been the domain of specialists and this is particularly true in the Council. However, the Council's new web based GIS tools will open up GIS to a far wider base of users, with a particular opportunity for data analysts to make more use of spatial data without being a GIS specialist.

A word of warning though before you jump in, the guidance, examples and functions targeted at helping you use our spatial data is still very much work-in-progress (*TODO*).
...
Listed below are R and Python packages that allow us to leverage ArcGIS content. Generally speaking, they are in ascending order of both ease of use and scope of functionality.
...
The `esri2sf` R package is the only one of the four packages listed above that isn't maintained by ESRI. It is the simplest to start using because it doesn't require an installation and license for ArcGIS Desktop or Pro. The flip side, is that it has the least amount of functionality and the code isn't as robust. The scope of the package is limited to reading simple features or data frames from AGOL & ArcGIS Enterprise (ours or other organisations).

The [ONS Open Geography Portal](https://geoportal.statistics.gov.uk/) is based on the same ArcGIS web tools as our Portal and AGOL. To get for example. the local authority boundary for Sheffield you first need to navigate to the [dataset](https://geoportal.statistics.gov.uk/datasets/local-authority-districts-december-2020-uk-bgc?geometry=-45.896%2C51.101%2C41.028%2C59.782) on the website to get the GeoService API URL:
...
> https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Local_Authority_Districts_December_2020_UK_BGC/FeatureServer/0/query?outFields=\*&where=1%3D1

2.  some resources to understand how to query via the API
...
The example above reads open data. Most of the Council's spatial data on Portal and AGOL requires login credentials to to access. Nevertheless, Portal and AGOL login credentials are a smaller requirement than a ArcGIS Desktop or Pro installation and license.

*TODO - test and note access via token authentication*

The current intention is to Where possible [scc-pi](https://github.com/scc-pi) functions have been written to use
...
The R-ArcGIS Bridge is the name for the `arcgisbinding` package.

Wrap R tools for use in ArcGIS
...
2.  The `arcpy` package is used to leverage the ArcGIS Desktop or Pro functionality.
