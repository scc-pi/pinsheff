...
If you find yourself copying and pasting the same code several times, that's a good indication that you should probably consider creating a function to do the copied code. Writing functions is an initial investment of time and effort, that pays off down the line, by making it easier to read and maintain scripts.    

The place to start getting a better understanding of the purpose of functions and how to write them, is the relevant chapter in [Hadley's R for Data Science](https://r4ds.had.co.nz/functions.html#functions). 

### Lazy & tidy evaluation  

A couple of concepts to be aware of when writing R functions are:  

1. Lazy evaluation  

1. Tidy evaluation  

Tidy evaluation in particular I'm not yet comfortable enough with yet to try and explain. I'm getting better at using it, but I've not fully understood it yet. [Hadley's Advanced R](https://adv-r.hadley.nz/) book is where you should look for a explanation:   
...
I've written a couple of small R packages called [popsheff](https://github.com/scc-pi/popsheff) and [boundsheff](https://github.com/scc-pi/boundsheff). Instead of functions, these contain population and boundary data for Sheffield. The resources I relied on to create these include:  
...
`install.packages` is usually used to install packages hosted on  [CRAN](https://cran.r-project.org/). Hosting a package on CRAN involves a verification process, quality check, reviews etc. I've not done that for the `popsheff` and `boundsheff` packages, so they have to ben installed a different way, either using:  
...
2. GitHub via the devtools R package and it's `install_github` function.  
