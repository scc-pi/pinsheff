...
To use GitHub with RStudio there's a thorough step-by-step guide [Happy Git with R](https://happygitwithr.com/) by Jenny Bryan. My main recommendation is to setup the connection using [keys for SSH](https://happygitwithr.com/ssh-keys.html). This connection method by-passes the issues I had with the Council's proxy server and using the HTTPS connection method. I've also had no issues since working from home and connecting via the Tunnel.

I frequently return to Jenny Bryan's book, especially for chapters 15, 16 and 17 on different ways of setting up a local RStudio project connected to GitHub. So far, I've mainly stuck with [Chapter 15 New project, GitHub first](Chapter 15 New project, GitHub first).