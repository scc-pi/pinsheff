...
`admin_district` can be used to answer the questions: is the postcode in Sheffield? is it a valid postcode? You can anticipate values of `Sheffield` and `NA` respectively if the answers are yes. These are common questions so the function has an `.admin_district` argument.   

Needing the coordinates of the postcode centroid is also common, so the function also has a `.lat_long` argument that if `positive `TRUE` returns a latitude column and a longitude column. 

_TODO: link to sf example & area stats_  
...
```{r, extract-pcd, message=FLASE, warning=FALSE}
# Create a data frame with some example records
df <- tribble(
  ~name,           ~address,
  "Bob Johnson",   "Pinstone St, Sheffield City Centre, Sheffield S1 2HH",
  "Chris Wilder",  "Bramall Ln, Highfield, Sheffield S2 4SU",
  "Neil Thompson", "76 Penistone Rd N, Sheffield S6 1SW"
)

# Extract the postcode from the address into a separate column
mutate(df, postcode = word(address, -2, -1, sep = "[:space:]"))  %>%
  knitr::kable()
```
### PAS Postcode Lookup  
...
_TODO: Does it also include Sheffield specific variables e.g. which of the 100 neighbourhoods is the postcode in?_    
